...
class ElementOptionType(Tile):
    
    def __init__(self, app, host, option_type):
        super().__init__()
        self.app: "MainWindow" = app
        self.host = host
        self.label: QLabel = self._label()
        self.value: QComboBox = self._value(option_type)
    ...
    def _value(self, option_type):
        select_box = QComboBox()
        select_box.hide()
        select_box.addItems(OPTION_CODES.keys())
        select_box.setCurrentText(option_type)
        ...
        select_box.currentTextChanged.connect(self.__selection_changed)
        return select_box
    ...
    def __selection_changed(self):
        ...
        target.clear()
        new_items = OPTION_CODES[text]["values"].keys()
        target.addItems(new_items)
        self.host.update_up()
        return
